...
import { CreateCollectionDto, UpdateCollectionDto, AddToCollectionDto, UpdateNoteDto } from './dto';
...
export class CollectionsController {
   ...
   */
  @Patch(':id')
  async updateCollection(@Param('id') id: string, @Request() req: any, @Body() dto: UpdateCollectionDto) {
    const userId = req.user?.id || '557be1bd-62cb-4125-a028-5ba740b66aca';
   ...
   */
  @Post(':id/items')
  async addToCollection(@Param('id') id: string, @Request() req: any, @Body() dto: AddToCollectionDto) {
    const userId = req.user?.id || '557be1bd-62cb-4125-a028-5ba740b66aca';
  ...
  async removeFromCollection(@Param('id') id: string, @Param('resourceId') resourceId: string, @Request() req: any) {
    const userId = req.user?.id || '557be1bd-62cb-4125-a028-5ba740b66aca';
  ...
  async updateNote(
    ...
    const userId = req.user?.id || '557be1bd-62cb-4125-a028-5ba740b66aca';
    return this.collectionsService.updateCollectionItemNote(id, resourceId, userId, dto.note);
  }
  ...
  async checkResource(@Param('resourceId') resourceId: string, @Request() req: any) {
    const userId = req.user?.id || '557be1bd-62cb-4125-a028-5ba740b66aca';
    return this.collectionsService.isResourceInUserCollections(userId, resourceId);
  }
}
