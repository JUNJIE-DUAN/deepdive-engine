...
export class NotesService {
  ...
  async createNote(userId: string, dto: CreateNoteDto) {
    ...
    this.logger.log(`Note created for resource ${dto.resourceId} by user ${userId}`);
  ...
  async removeHighlight(noteId: string, userId: string, highlightId: string) {
    ...
    // 移除高亮
    const currentHighlights = (note.highlights as any[]) || [];
    const filteredHighlights = currentHighlights.filter((h: any) => h.id !== highlightId);
    ...
    try {
      ...
      const response = await fetch(`${aiServiceUrl}/api/v1/ai/chat`, {
        ...
        body: JSON.stringify({
          ...
          context: note.resource ? `资源标题: ${note.resource.title}\n摘要: ${note.resource.abstract || '无'}` : undefined,
          model: 'grok',
          stream: false,
        }),
      });
      ...
      this.logger.error(`Failed to call AI service: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
   ...
   * 关联知识图谱节点
   */
  async linkGraphNode(noteId: string, userId: string, nodeId: string, nodeType: string) {
    // 验证所有权
    const note = await this.prisma.note.findUnique({
      where: { id: noteId },
    });
