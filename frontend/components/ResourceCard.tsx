...
import { useThumbnailGenerator, needsThumbnail } from '@/lib/use-thumbnail-generator';
...
export default function ResourceCard({ resource, onClick, onToggleBookmark, isBookmarked }: ResourceCardProps) {
  const [localThumbnailUrl, setLocalThumbnailUrl] = useState<string | null>(resource.thumbnailUrl || null);
  ...
  useEffect(() => {
    const autoGenerateThumbnail = async () => {
      if (needsThumbnail(resource) && !isGeneratingThumbnail) {
        ...
        try {
          const success = await generateAndUploadThumbnail(resource.id, resource.pdfUrl!);

          if (success) {
            // Fetch updated resource to get thumbnail URL
            const response = await fetch(`${config.apiBaseUrl}/api/v1/resources/${resource.id}`);
            if (response.ok) {
