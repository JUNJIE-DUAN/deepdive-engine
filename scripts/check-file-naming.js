...
class FileNamingChecker {
  ...
  traverse(dir) {
    ...
    try {
      ...
      console.error(colorize(`Error reading directory ${dir}: ${error.message}`, 'red'));
      return;
    }
      ...
      if (entry.isDirectory()) {
        ...
        if (violation) {
          this.violations.push(
            new Violation('directory', violation.currentPath, violation.suggestedPath, violation.reason)
          );
        }
        ...
        if (violation) {
          this.violations.push(
            new Violation('file', violation.currentPath, violation.suggestedPath, violation.reason)
          );
        }
      }
    }
  ...
  run() {
    ...
    console.log(`检查规则：所有文件名必须小写（${this.config.rootExceptions.length} 个例外）\n`);

    for (const dir of this.config.checkDirs) {
      if (fs.existsSync(dir)) {
        console.log(`${colorize('🔍', 'yellow')} 检查目录: ${colorize(dir, 'cyan')}`);
        this.traverse(dir);
      } else {
  ...
  printResults(violations) {
    ...
    console.log(colorize('   ./scripts/rename-docs-lowercase.sh --dry-run', 'cyan'));
  ...
  generateFixScript(violations, outputPath = 'scripts/auto-rename.sh') {
    ...
    lines.push('echo "  git commit -m \\"refactor: fix file naming violations\\""');
